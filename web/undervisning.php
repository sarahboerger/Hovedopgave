...
<head>
  ...
  <meta name="keywords" content="Enggaarden,Rideskole,Enggaarden Skolen for Akademisk Ridekunst,Akademisk Ridekunst">
    ...
    <title>Undervisning | Enggaarden Skolen for Akademisk Ridekunst</title>
</head>
...
<div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12 text-center">
                    <h2>Velkommen til undervisning!</h2>
                    <p>Dette forum kan bruges til at diskutere alle former for undervisning.</p>
                </div>
            </div>

<form action="undervisning.php" method="post">
    <?php
            ...
            if($_SERVER['REQUEST_METHOD'] === 'POST'){
                ...
                if($_REQUEST['knap'] == "read"){
                    ...
                    if(is_numeric($personID)){}
                        $sql = $conn->prepare("select * from undervisning where personID = ?");
                ...
                if($_REQUEST['knap'] == "update"){
                    ...
                    $besked = $_REQUEST['besked'];
                            if(is_numeric($personID)){
                            $sql = $conn->prepare("update undervisning set navn = ?, besked = ? where personID = ?");
                ...
                //Create - VIRKER
                      if($_REQUEST['knap'] == "create"){
                        ...
                        $besked = $_REQUEST['besked'];
                            if(is_numeric($personID)){
                            $sql = $conn->prepare("insert into undervisning (personID, navn, besked) values (?, ?, ?)");
                    ...
                    //Delete - VIRKER
                     if($_REQUEST['knap'] == "delete"){
                        $personID = $_REQUEST['personID'];
                                if(is_numeric($personID)){
                                $sql = $conn->prepare("delete from undervisning where personID = ?");
