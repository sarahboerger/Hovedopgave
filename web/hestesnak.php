...
<head>
  ...
  <meta name="keywords" content="Enggaarden,Rideskole,Enggaarden Skolen for Akademisk Ridekunst,Akademisk Ridekunst">
    ...
    <title>Hestesnak | Enggaarden Skolen for Akademisk Ridekunst</title>
</head>
...
<div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12 text-center">
                    <h2>Velkommen til hestesnak!</h2>
                    <p>Dette forum kan bruges til at diskutere alle former for hestesnak.</p>
                </div>
            </div>

<form action="hestesnak.php" method="post">
    <?php
            ...
            if($_SERVER['REQUEST_METHOD'] === 'POST'){
                ...
                if($_REQUEST['knap'] == "read"){
                    ...
                    if(is_numeric($personID)){}
                        $sql = $conn->prepare("select * from hestesnak where personID = ?");
                ...
                if($_REQUEST['knap'] == "update"){
                    ...
                    $besked = $_REQUEST['besked'];
                            if(is_numeric($personID)){
                            $sql = $conn->prepare("update hestesnak set navn = ?, besked = ? where personID = ?");
                ...
                //Create - VIRKER
                      if($_REQUEST['knap'] == "create"){
                        ...
                        $besked = $_REQUEST['besked'];
                            if(is_numeric($personID)){
                            $sql = $conn->prepare("insert into hestesnak (personID, navn, besked) values (?, ?, ?)");
                    ...
                    //Delete - VIRKER
                     if($_REQUEST['knap'] == "delete"){
                        $personID = $_REQUEST['personID'];
                                if(is_numeric($personID)){
                                $sql = $conn->prepare("delete from hestesnak where personID = ?");
