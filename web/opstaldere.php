...
<head>
  ...
  <meta name="keywords" content="Enggaarden,Rideskole,Enggaarden Skolen for Akademisk Ridekunst,Akademisk Ridekunst">
    ...
    <title>Opstaldere | Enggaarden Skolen for Akademisk Ridekunst</title>
</head>
...
<div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12 text-center">
                    <h2>Velkommen til opstalderene!</h2>
                    <p>Dette forum kan bruges til at diskutere alle former for opstaldning.</p>
                </div>
            </div>
            ...
            if($_SERVER['REQUEST_METHOD'] === 'POST'){
                ...
                if($_REQUEST['knap'] == "read"){
                    ...
                    if(is_numeric($personID)){}
                        $sql = $conn->prepare("select * from opstaldere where personID = ?");
                ...
                if($_REQUEST['knap'] == "update"){
                    ...
                    $besked = $_REQUEST['besked'];
                            if(is_numeric($personID)){
                            $sql = $conn->prepare("update opstaldere set navn = ?, besked = ? where personID = ?");
                ...
                //Create - VIRKER
                      if($_REQUEST['knap'] == "create"){
                        ...
                        $besked = $_REQUEST['besked'];
                            if(is_numeric($personID)){
                            $sql = $conn->prepare("insert into opstaldere (personID, navn, besked) values (?, ?, ?)");
                    ...
                    //Delete - VIRKER
                     if($_REQUEST['knap'] == "delete"){
                        $personID = $_REQUEST['personID'];
                                if(is_numeric($personID)){
                                $sql = $conn->prepare("delete from opstaldere where personID = ?");
